...
export async function generateMetadata({
  ...
  const game: Game | null = await getPendingGameById(params.id);
...
export default async function PendingGameDetailPage({
  params,
}: {
  params: { id: string };
}) {
  const game = await getPendingGameById(params.id);
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3 mb-4">
            ...
            {game.isEarlyAccess === 1 && (
              <Badge className="bg-blue-500">얼리 액세스</Badge>
            )}
          </div>
